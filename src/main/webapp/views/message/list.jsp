...
<display:table name="messages" id="row" requestURI="${requestURI}"
	...
	<jstl:if test="${requestURI == 'message/actor/listR.do'}">
		<display:column>
			<input
		...
		<display:column>
			...
			confirm('<spring:message code="message.confirm.delete" />');
			 window.location.replace('message/actor/deleteReceived.do?id=${row.id}');"
				value="<spring:message code="message.delete" />" type="button" />
		</display:column>

	</jstl:if>

	<jstl:if test="${requestURI == 'message/actor/listS.do'}">

		<display:column>
			<input
				onclick="javascript:
			confirm('<spring:message code="message.confirm.delete" />');
			 window.location.replace('message/actor/deleteSent.do?id=${row.id}');"
				value="<spring:message code="message.delete" />" type="button" />
		</display:column>
